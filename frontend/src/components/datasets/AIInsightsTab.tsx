  ...
  return (
    <div className="space-y-6 animate-pulse pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="h-44 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)]" />
        <div className="h-44 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)]" />
        <div className="h-44 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)]" />
        <div className="h-44 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)]" />
      </div>
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-red-500/30 bg-red-500/10 p-8 text-center max-w-xl mx-auto shadow-xl">
        ...
        <p className="text-sm text-[var(--color-text-muted)] mb-6 leading-relaxed">{error || 'Unable to audit dataset schema.'}</p>
        <button
          onClick={fetchInsights}
      ...
      <div className="rounded-2xl border border-purple-500/40 bg-gradient-to-br from-purple-500/15 via-blue-500/10 to-transparent p-6 sm:p-8 shadow-2xl relative">
        <div className="flex flex-col xl:flex-row xl:items-start justify-between gap-6">
          ...
          <div className="shrink-0 bg-black/50 border border-[var(--color-border)] rounded-xl p-5 max-w-md space-y-2.5 shadow-inner">
            ...
            <div className="text-xs sm:text-sm leading-relaxed text-[var(--color-text-secondary)] font-normal pt-1">
      ...
      {showFullProfile && (
        <div className="mt-8 space-y-6 pt-8 border-t border-[var(--color-border)] animate-in slide-in-from-top-2 duration-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="rounded-2xl border border-[var(--color-border)] bg-black/40 p-6 space-y-4 shadow-lg">
              <h4 className="text-base font-bold text-[var(--color-text-primary)] border-b border-[var(--color-border)] pb-3">
            ...
            <div className="rounded-2xl border border-[var(--color-border)] bg-black/40 p-6 space-y-4 shadow-lg overflow-hidden">
              <h4 className="text-base font-bold text-[var(--color-text-primary)] border-b border-[var(--color-border)] pb-3">
              ...
              <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                {observationsList.length > 0 ? (
                  observationsList.map((obs, idx) => (
                    <div key={idx} className="bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-xl p-3.5 space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-bold text-purple-300 font-mono">{obs.feature}</span>
                      </div>
                      <p className="text-xs text-[var(--color-text-secondary)] leading-relaxed">{obs.observation}</p>
                    </div>
                  ))
                ) : (
